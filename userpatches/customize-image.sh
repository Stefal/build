...
# disable wifi which is not reliable and use trace_printk commands
echo 'blacklist uwe5622_bsp_sdio' > /etc/modprobe.d/wifi_uwe_5622.conf
echo 'install uwe5622_bsp_sdio /bin/true' >> /etc/modprobe.d/wifi_uwe_5622.conf
# add red led blinking on activity
...
#wget -4 https://rtkbase.eu/rtkbase_beta/install.sh
#wget -4 https://raw.githubusercontent.com/stefal/rtkbase/python_package_update/tools/install.sh
chmod +x install.sh
...
#./install.sh --user ${_user} --dependencies --rtklib --rtkbase-repo master --gpsd-chrony
#./install.sh --user ${_user} --dependencies --rtklib --rtkbase-bundled --gpsd-chrony
#./install.sh --user ${_user} --dependencies --rtklib --rtkbase-repo python_package_update --gpsd-chrony
# changing hostname
